...
public sealed partial class PrayerSystem : EntitySystem
{
    ...
    [Dependency] private IAdminManager _admin = default!; // Goobstation - Admin Notifications
    ...
    public void Pray(ICommonSession sender, PrayableComponent comp, string message)
    {
        ...
        // <Trauma>
        if (comp.NotificationSound is {} sound)
        {
            foreach (var admin in _admin.ActiveAdmins)
            {
                RaiseNetworkEvent(new AdminNotificationEvent(sound), admin);
            }
        }
        // </Trauma>
    }
