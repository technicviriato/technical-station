...
public sealed partial class MaterialStorageControl : ScrollContainer
{
    ...
    protected override void FrameUpdate(FrameEventArgs args)
    {
        ...
        // <Goob>
        var storage = materialStorage.Storage.Select(pair => (pair.Key.Id, pair.Value)).ToDictionary();
        SiloLabel.Visible = false;
        if (_silo.GetSilo(_owner.Value) is { } silo)
        {
            if (!_entityManager.TryGetComponent<MaterialStorageComponent>(silo, out var siloComp))
                return;
            storage = siloComp.Storage.Select(pair => (pair.Key.Id, pair.Value)).ToDictionary();
            SiloLabel.Visible = true;
        }
